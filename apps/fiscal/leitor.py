...
def _participante(elemento, *, apenas_cnpj_ou_cpf=False):
    ...
    if cnpj is not None:
        return ParticipanteLido("CNPJ", cnpj, _texto(elemento, "n:xNome") or "")
    cpf = _texto(elemento, "n:CPF")
    if cpf is not None:
        return ParticipanteLido("CPF", cpf, _texto(elemento, "n:xNome") or "")
    if apenas_cnpj_ou_cpf:
        raise ArquivoRecusado("Prestador sem CNPJ nem CPF.")
    nif = _texto(elemento, "n:NIF")
    if nif is not None:
        return ParticipanteLido("NIF", nif, _texto(elemento, "n:xNome") or "")
    nao_nif = _texto(elemento, "n:cNaoNIF")
    if nao_nif is not None:
        return ParticipanteLido("nao_informado", "", _texto(elemento, "n:xNome") or "")
...
def _ler_evento(raiz, conteudo: bytes, sha256: str) -> EventoLido:
    ...
    if cnpj_autor is not None:
        autor = ParticipanteLido("CNPJ", cnpj_autor, "")
    elif cpf_autor is not None:
        autor = ParticipanteLido("CPF", cpf_autor, "")
    ...
    # DIRETOS de infPedReg e pegamos o primeiro cujo nome local bate com o
    # formato; os demais filhos (tpAmb, verAplic, dhEvento, CNPJAutor/
    # CPFAutor, chNFSe) nunca casam com "e" + 6 dígitos, então não há
    # ambiguidade em percorrer todos em vez de exigir a posição exata.
    codigo = None
