...
def test_pagina_real_balancete_nao_fecha_com_totais_forcados(
    ...
    resposta = client.get(_url_balancete(cenario_pagina_real, inicio=hoje.replace(day=1), fim=hoje))
    # DL-027 Fatia B: o ramo "não fecha" virou VETO — 409 sem renderizar
    # a página imprimível, suas linhas ou o carimbo B.3.
    assert resposta.status_code == 409
    html = resposta.content.decode()
    assert "<table" not in html
    assert "Emitido em" not in html
    assert "Empresa Veredito na Página Ltda" not in html
