...
6. Reporta no stdout os totais: lidos, competências criadas, FKs
   atribuídas, órfãos pulados, duração.
...
2. **Concorrência segura:** novos lançamentos criados durante o
   ...
   isso pode acontecer.
3. **Órfãos são pulados, não falham:** lançamentos com `empresa_id`
   nula ou apontando para empresa inexistente viram contador "órfãos",
   são logados, e o command termina com exit code 0 — não 1. O
   relatório fica para revisão manual antes do `--apply` em produção.
...
6. **CI verde:** suíte de testes nova em
   ...
   os 4 cenários abaixo.

## Cenários de teste

| Cenário | Setup | Expectativa |
|---|---|---|
| **Órfão pulado** | Lançamento com `empresa=None` (não deve existir em produção, mas o teste força) | Não vira `Competencia`, contador `orfãos` += 1, exit 0 |
...
| **Dry-run não altera** | Lançamento sem FK, rodar com `--dry-run` (default) | `Competencia` count não muda, FK permanece `None`, exit 0 |
...
2. **Qualidade dos dados.** Lançamentos com `empresa_id` nula ou
   apontando para empresa inexistente viram "órfãos". O command
   deve contar, logar e pular (não falhar). Antes do `--apply` em
   produção real, revisar o relatório.
3. **Corrida com `criar_lancamento`.** Lançamentos criados durante
...
4. **Reversibilidade.** Aplicada a FK não volta atrás com a command
