...
| [DL-016](../planos/DL-016-competencia-e-fechamento.md) | Competência e fechamento de período, com reabertura autorizada e auditada | **Onda 1 (F1 + F2) em revisão na CI — PR #31, segundo push (`81179b0`).** Modelo `Competencia` com `EstadoCompetencia` (aberta, em_encerramento, encerrada), `UniqueConstraint(empresa, ano, mes)` e checks de faixa (mês 1..12, ano 1970..2999). FK `LancamentoContabil.competencia` (PROTECT, nullable até F5) preenchida por `get_or_create` em `services.criar_lancamento` com savepoint próprio. 7 testes do modelo — validados por `python -m py_compile` mas **não executados** no turno de escrita (sandbox Python 3.11, projeto exige 3.12+); execução é responsabilidade da CI. **Segunda rodada da CI reprovou em mais 5 testes** (1353 pass, 2 skipped): dois eram bugs reais do meu teste (`assertRaises(IntegrityError)` sem savepoint corrompia a transação do TestCase no segundo `subTest`, e `.values_list(...).order_by()` com `order_by()` vazio cancelava o `Meta.ordering`); corrigidos. Os outros dois eram os testes de **varredura** da DL-019 e DL-023 detectando que as 3 constraints e o model novo no admin não estavam registrados — corrigidos em `apps/core/restricoes.py:204-237` e `apps/core/tests/test_dl023_varredura_admin.py:166`, com DE-047 documentando o porquê de `RESTRICOES_SEM_CAMINHO_DE_CLIENTE` e categoria `defendida`. DE-046 (fixture) e DE-044/045 (lint+gate) permanecem aplicados. F3/F4/F5 fora do escopo deste PR |
