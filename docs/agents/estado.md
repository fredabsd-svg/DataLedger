...
| [DL-027](../planos/DL-027-documento-emitido-e-personalizacao.md) | O documento emitido: identificação obrigatória por **classe de documento** e personalização do que é legítimo personalizar. Mecanismo de **plataforma**, não da Contabilidade — vale para todos os módulos (RC-94) | **Integrada (Fatia A — Identificação, PR #39, `e84bcfc`)** — enum `ClasseDocumento` (Conferência/Demonstração/Livro), função pura `bloco_obrigatorio_para` por classe com regra do vazio `(não informado)`, campos `nire` e `nivel_de_arredondamento` no cadastro de Empresa (RC-93 + NBC TG 26 item 51e), migração `0006_empresa_nire_empresa_nivel_de_arredondamento`, `apps.documentos` no `INSTALLED_APPS`. 4/4 CI checks verdes no merge, suíte local 443 passed nos arquivos da fatia. Próximas fatias da mesma etapa: **B — Preferências sem imagem** (carimbo, critério impresso, bloqueio quando não fecha, marca d'água, ocultar sem movimento), **C — Logotipo** e **D — Pré-visualização**. Item 5 do plano da Fatia A — guarda que reprova classe não declarada em templates imprimíveis — ficou **fora do escopo desta entrega** e entra como trabalho conjunto à Fatia B (a varredura precisa de templates que mudem de comportamento com a preferência ligada, e por isso a ordem original — A → B → C → D — permanece a recomendação) |
