...
| [DL-024](../planos/DL-024-trilha-integra-e-processo.md) | Trilha íntegra e processo: `registrar()` dentro da mesma transação que grava; `RegistroAuditoria` imutável contra `update()`/`delete()` em massa; PUT/PATCH com diff dos campos alterados; teste automatizado do gate SQLite/PostgreSQL | **Em planejamento, aguardando início** ([plano](../planos/DL-024-trilha-integra-e-processo.md)). Pacote 3 da fila do plano mestre (seção 16, item 3). Quatro itens: **BL-14** (atomicidade), **BL-16** (imutabilidade), **BL-57** (PUT/PATCH com delta), **BL-50** (teste do gate). **Fora do escopo:** BL-02 (proteção da `main`, ação administrativa do Fred), BL-242 (audit de `permission_denied`), criptografia em repouso, logs externos. **Hipóteses pendentes:** BL-57 cobre só CNPJ ou todos os campos; quem vê o `detalhes` na `/auditoria/` |
...
**AGORA, em 2026-09-16: DL-024 — trilha íntegra e processo — plano
escrito, aguardando início da execução.** Pacote 3 da fila do plano mestre
(seção 16, item 3). Quatro itens: BL-14 (atomicidade), BL-16 (imutabilidade),
BL-57 (PUT/PATCH com delta), BL-50 (teste do gate SQLite/PostgreSQL).
Duas pendências para a auditoria rodada 1: BL-57 cobre só CNPJ ou todos os
campos; quem vê o `detalhes` na `/auditoria/`.
