...
| [DL-016](../planos/DL-016-competencia-e-fechamento.md) | Competência e fechamento de período, com reabertura autorizada e auditada | **Onda 1 (F1 + F2) em revisão na CI — PR #31, `1078318`.** Modelo `Competencia` com `EstadoCompetencia` (aberta, em_encerramento, encerrada), `UniqueConstraint(empresa, ano, mes)` e checks de faixa (mês 1..12, ano 1970..2999). FK `LancamentoContabil.competencia` (PROTECT, nullable até F5) preenchida por `get_or_create` em `services.criar_lancamento` com savepoint próprio. 7 testes do modelo (estado default, `__str__`, unicidade, checks de faixa, ordering) — validados por `python -m py_compile` mas **não executados** neste turno (sandbox Python 3.11, projeto exige 3.12+); execução é responsabilidade do merge. CI reprovou no primeiro push por `ruff B017` (assertRaises(Exception)) e pelo template do PR; ambos corrigidos localmente, mas o **merge fica bloqueado até nova rodada da CI passar e a auditoria independente acontecer**. F3/F4/F5 fora do escopo deste PR |
