...
| [DL-016](../planos/DL-016-competencia-e-fechamento.md) | Competência e fechamento de período, com reabertura autorizada e auditada | **Onda 1 (F1 + F2) em revisão na CI — PR #31, `bb23339`.** Modelo `Competencia` com `EstadoCompetencia` (aberta, em_encerramento, encerrada), `UniqueConstraint(empresa, ano, mes)` e checks de faixa (mês 1..12, ano 1970..2999). FK `LancamentoContabil.competencia` (PROTECT, nullable até F5) preenchida por `get_or_create` em `services.criar_lancamento` com savepoint próprio. 7 testes do modelo (estado default, `__str__`, unicidade, checks de faixa, ordering) — validados por `python -m py_compile` mas **não executados** no turno de escrita (sandbox Python 3.11, projeto exige 3.12+); execução é responsabilidade da CI. **Primeira rodada da CI reprovou em setup** com `psycopg.errors.NotNullViolation: null value in column "escritorio_id" of relation "empresas_empresa"` porque o `setUpTestData` criava `Empresa` sem `escritorio` (FK `NOT NULL` desde DL-009); corrigido em commit posterior adicionando `Escritorio.objects.create(...)` antes da `Empresa`, mesmo padrão de `test_services.py` e `test_dl024_*.py` — ver DE-046. **Correção anterior de lint (`ruff B017` → `pytest.raises(Exception)`) e DE-044 (gate regex sem `**`) permanecem aplicadas.** F3/F4/F5 fora do escopo deste PR |
