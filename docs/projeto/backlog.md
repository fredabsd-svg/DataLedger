...
| BL-63 | **Saldo inicial de implantação.** Não existe. Empresa que chega ao escritório com contabilidade em andamento não tem como entrar no sistema sem inventar lançamentos. | `arquiteto-senior` define; `desenvolvedor-pleno` implementa | PE-27 | **bloqueada** — depende da resposta do Fred (PE-27) | Saldo inicial por conta, com data de implantação, distinguível de movimento do período; o Balancete o apresenta como saldo anterior; a soma dos saldos iniciais devedores é igual à dos credores. |
| BL-64 | **Conferência de lotes com diferença entre débito e crédito.** A igualdade é validada no serviço, mas nada varre a base: um lote gravado por caminho que não passe pelo serviço (migração, shell, importação futura) fica torto em silêncio. | `desenvolvedor-pleno` | — | planejada | Consulta que lista lotes desbalanceados da empresa, com a diferença; teste que grava um lote torto por caminho alternativo e prova que a consulta o encontra. |
| BL-65 | **Correção em massa como lote de ajuste rastreável.** O sistema de referência sobrescreve lançamentos efetivados por filtro amplo; as nossas regras proíbem. A dor é real (corrigir centenas de classificações à mão é inviável) e precisa de solução própria. | `arquiteto-senior` define; `desenvolvedor-pleno` implementa | PE-33, BL-10 | **bloqueada** — depende da resposta do Fred (PE-33) | Correção em lote gera estornos e relançamentos vinculados ao original, com autor, data e motivo; nenhum lançamento efetivado é alterado; a operação é atômica e idempotente; teste com falha no meio do lote provando que nada fica pela metade. |
| BL-66 | **Eliminação de período: decidir não fazer, e registrar por quê.** O sistema de referência descarta lançamentos antigos por data. Isso conflita com a trilha permanente. | `arquiteto-senior` | — | planejada | Decisão registrada em [decisoes.md](decisoes.md): não implementar exclusão; se houver demanda de volume, tratar como arquivamento com inventário e caminho de volta. |
