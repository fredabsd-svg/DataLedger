...
| BL-153 | **R6-6: regime tributário com vigência no ano 9999 é porta de mão única.** `date.max` não tem data posterior, a regra exige vigência crescente, e não há `PUT`/`DELETE` nem edição na tela: **a empresa nunca mais pode ter regime registrado**, e só acesso direto ao banco desfaz. Um dígito errado congela para sempre o histórico do dado que governa a apuração fiscal. | `desenvolvedor-pleno` | — | planejada | **A classe é:** nenhum erro de digitação deixa um cadastro sem caminho de correção pelo produto. *Exemplo*: faixa superior para `vigencia_inicio` **e/ou** caminho auditado de correção do último período, no molde do estorno (registrar a correção, nunca editar em silêncio). A faixa é decisão do Fred. |
