...
| BL-63 | **Lançamento de abertura (saldos iniciais).** RC-53: os saldos iniciais entram como **lançamento dos saldos do balanço patrimonial**, não como cadastro à parte. Falta o assistente que torna isso praticável e seguro. | `desenvolvedor-pleno` | BL-59 | planejada | Lançamento de abertura identificado como tal, com data de implantação; recusa se ativo ≠ passivo + patrimônio líquido; recusa conta de resultado (saldo inicial de resultado é sempre zero — o exercício anterior já foi encerrado); aparece no Balancete como saldo anterior do primeiro período; teste com balanço real de três níveis. |
...
| BL-66 | **Eliminação de período** (RC-52, salvaguardas em **DE-018**). | `desenvolvedor-pleno` | **BL-33** (restauração testada), **BL-11**, PE-34 | **bloqueada** — a ordem é deliberada: apagar dado de cliente antes de haver restauração provada é aposta, não funcionalidade | Só período encerrado; exportação completa **verificada** antes de apagar, e falha na exportação aborta tudo; papel autorizado com confirmação mostrando contagem e totais; inventário permanente do que foi eliminado (com impressão digital do arquivo), que nenhuma eliminação posterior apaga; teste que prova que a eliminação é abortada quando a exportação falha. |
