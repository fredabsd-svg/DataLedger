...
| BL-64 | **Conferência de lotes com diferença entre débito e crédito.** A igualdade é validada no serviço, mas nada varre a base: um lote gravado por caminho que não passe pelo serviço (migração, shell, importação futura) fica torto em silêncio. | `desenvolvedor-pleno` | — | **implementada em `8f2c209`, REPROVADA na auditoria** — em correção | Consulta que lista lotes desbalanceados da empresa, com a diferença; teste que grava um lote torto por caminho alternativo e prova que a consulta o encontra. |
