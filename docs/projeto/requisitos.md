...
| PE-44 | **Regime tributário com vigência no ano 9999 é porta de mão única.** Achado R6-6 da [rodada 6](../auditorias/2026-09-15-dl-017-rodada-6.md): a regra exige vigência crescente, `date.max` não tem data posterior, e não existe `PUT`, `DELETE` nem edição na tela — então **a empresa nunca mais pode ter regime registrado** e só acesso direto ao banco desfaz. Duas perguntas: (a) qual é a **faixa aceitável** de `vigencia_inicio` de um regime tributário — o escritório registra regime com vigência futura, e de quanto tempo? (b) quando o contador erra a vigência, a correção deve **apagar** o registro errado ou **registrar uma correção** que o substitui, no molde do estorno? | A faixa é regra de rotina, não técnica: um escritório que registra o Simples do ano seguinte em dezembro precisa de vigência futura; um que só registra o vigente não precisa de nenhuma. Presumir a faixa é o que o `AGENTS.md` proíbe. A alínea (b) decide se a trilha de auditoria guarda o erro ou não — e isso não é reversível depois de escolhido. | Levantado pelo `arquiteto-senior` em 2026-09-15 ao distribuir a DL-019. **Enquanto não houver resposta**, a BL-153 entrega só a parte que não depende da decisão: o caminho auditado de correção do último período, que torna qualquer erro de digitação recuperável **independentemente** da faixa. Nenhuma faixa superior é inventada. |
